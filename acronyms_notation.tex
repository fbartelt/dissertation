...
\newnotation{Smapsothree}{$\widehat{\mathcal{S}}$}{Canonical isomorpshism that maps $\mathbb{R}^3$ to $\mathfrak{so}(3)$}
...
\newnotation{Loppart}{$\text{L}_\mathbf{V}, \text{L}_\mathbf{W}$}{L operator for a function of two variables, performing the variations on the first and second variable, respectively}
\newnotation{chainruleLop}{$\mathcal{Z}(\mathbf{Y})$}{Part of the chain rule for the L operator in $\text{SE}(3)$}
\newnotation{XIoperator}{$\Xi$}{Maps a differentiable function $\mathbf{G}:\mathbb{R}\to G$ into a function $\Xi[\mathbf{G}]:\mathbb{R}\to\mathbb{R}^m$}
...
\newnotation{Rnnplus}{$\mathbb{R}^{n\times n}_+$}{Set of $n\times n$ with positive eigenvalues}
