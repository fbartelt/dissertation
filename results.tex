...
\vtwo{Before specifying the performed simulation in $\text{SO}^+(3, 1)$ it is necessary to introduce some concepts on special relativity. Relativistic motions occur in the Minkowski space, a four-dimensional description of spacetime. Elements of this space are called \emph{events}, represented as $\mathbf{p}=[x\ y \ z\ t]^\top\in\mathbb{R}^4$, where $x$, $y$ and $z$ are the spatial coordinates and $t$ is the time coordinate. Since time dilation and length contraction are effects experienced by observers in relative motion, a transformation of rerentials is required to relate the coordinates of events in different frames. These transformations are called \emph{Lorentz transformations}.}

\vtwo{Lorentz transformations are matrices that belong to the \emph{Lorentz group}. These transformations include spatial rotations, and reflections, and \emph{boosts} \citep[p. 12]{Carroll2019}, which is a transformation in Minkowski space without rotation. Specifically, the proper orthochronous Lorentz group $\text{SO}^+(3, 1)$ is the subgroup of the Lorentz group that includes only rotations and boosts that preserve the direction of time. The elements of $\text{SO}^+(3, 1)$ are $4\times 4$ matrices that preserve the \emph{Minkowski metric}:}
\vtwo{
\begin{align}
    \nu^2 = x^2 + y^2 + z^2 - t^2,
\end{align}
where $\nu$ is called the \emph{spacetime interval}.}

\vtwo{If an observer $O_2$ moves near the speed of light, it will experience an event with different coordinates $\mathbf{p}' = [x'\ y'\ z'\ t']^\top$ compared to the inertial observer $O_1$. These coordinates are related to $\mathbf{p}$ by the transformation $\mathbf{p}' = \boldsymbol{\Lambda}\mathbf{p}$, where $\boldsymbol{\Lambda}$ is a Lorentz transformation. For example, if $O_2$ boosts along the $x$-axis with a velocity $v$, the Lorentz transformation is given by
\begin{align}
   \begin{split}
     x' &= \gamma(x - vt)\\
     y' &= y\\
   ...
   \end{split}
\end{align}
where $c$ is the speed of light and $\gamma = \frac{1}{\sqrt{1 - v^2/c^2}}$ is the \emph{Lorentz factor}.}

\vtwo{As can be seen in \eqref{eq:results-lorentz-example}, observer $O_2$ will experience time dilation if it moves near the speed of light. It is the common to think about the different ``times'' as if each reference frame had an attached clock that ticks at a different rate. The time measured by a clock at the inertial frame $O_1$ is denoted as the \emph{coordinate time}, while the time measured by a clock at the relativistic frame $O_2$ is called the \emph{proper time}. The proper time $\tau$ is related to the coordinate time $t$ by $\frac{dt}{d\tau} = \gamma$ \citep[p. 32]{steane2012relativity}.}

\vtwo{With all these concepts introduced, we can now present the simulation performed in $\text{SO}^+(3, 1)$. We henceforth adopt $c=1$, as it is usual in special relativity, which implies that all quantities are adimensional.}

\subsubsection{Target curve}
\vtwo{The target curve $\mathcal{C}\in\text{SO}^+(3, 1)$ was taken as a periodic boost along the $x$-axis. The parametrization of the curve $\mathbf{H}_d$ was defined as:
\begin{align}
    \mathbf{H}_d(s) = \begin{bmatrix}
...
\end{align}}
\vtwo{This curve was discretized into $P=\num{30000}$ points by sampling $s$ into $\num{30000}$ evenly spaced points within the interval $[0, 1]$. To distinguish the continuous and discrete parametrizations, we again use the notations $\mathbf{H}_d[i]$ for the $i$-th sample of the discretized curve and $\mathbf{H}_d(s)$ for the continuous curve.}

\subsubsection{Choice of S map}
\vtwo{The chosen S map was the same as the one shown in \cref{ex:lorentz-group-lie-algebra}, thus for a vector $\boldsymbol{\xi}=[\xi_1\ \xi_2\ \xi_3\ \xi_4\ \xi_5\ \xi_6 ]^\top$, the corresponding isomorphism $\mathcal{S}:\mathbb{R}^6\to\mathfrak{so}(3,1)$ is given by:
\begin{align}
    \SL[\boldsymbol{\xi}] = \begin{bmatrix}
        0 & \xi_1 & \xi_2 & \xi_4 \\
        -\xi_1 & 0 & \xi_3 & \xi_5 \\
...
\end{align}}
\subsubsection{Employed numerical methods}
\vtwo{The optimization problem in \eqref{eq:optimization-problem-distance-definition-point-curve} was solved using the discretized curve, and the nearest point $\mathbf{H}_d[i^*]$ was computed through a brute-force approach. The derivative $\mathbf{H}_d'[i]$ was taken as the explicit derivative of the discrete version of \cref{eq:results-lorentz-curve-parametrization}. The normal component was approximated using the same method as in the previous simulation, and thus the $i^{th}$ normal component element $\boldsymbol{\xi}_{N,i}$ as:
\begin{align}
    \boldsymbol{\xi}_{N,i} \approx \frac{\widehat{D}\Bigl(\exp\bigl(\SL[\boldsymbol{\mathbf{e}}_i]\bigr)\mathbf{H}(t), \mathbf{H}_d[s^*]\Bigr) - \widehat{D}\bigl(\mathbf{H}(t), \mathbf{H}_d[s^*]\bigr)}{\varepsilon},
\end{align} 
in which $\varepsilon = \num{1E-4}$ was used.}
...
\subsubsection{Vector field gains and initial condition}
\vtwo{The gains were selected using the same reasoning as in the previous simulation, ensuring that the normal component would dominate when the system was far from the curve, and the tangent component would dominate when the system was near the curve. The following gains were chosen:
\begin{align}
    k_N(\mathbf{H}) &= \tanh\bigl(\num{1000}D(\mathbf{H})\bigr)\\
    k_T(\mathbf{H}) &= 0.5\Bigl(1 - \bigl(100D(\mathbf{H})\bigr)\Bigr).
\end{align}}

\vtwo{The initial condition $\mathbf{H}(0)$ was set as a boost along the $x$-axis, but in the opposite direction:
\begin{align}
    \mathbf{H}(0) = \exp\bigl(\SL[\boldsymbol{\xi}_0]\bigr),
\end{align}
where $\boldsymbol{\xi}_0 = [0\ 0\ 0\ 0.7\ 0\ 0]^\top$.}
\subsubsection{Simulation}
\vtwo{As it is impossible to plot the behavior of the system in $\text{SO}^+(3, 1)$ directly, we chose to provide visualizations based on special relativity. As such, we first simulate a circular motion in the inertial frame, and then we simulate the same motion in a relativistic frame, given by the Lorentz transformation of the current state. This circular motion is described by:}
\begin{align}
    \begin{split}
        \mathbf{p}_c &= [x_c\ y_c\ z_c\ t_c]^\top\\
        x_c &= 15\cos\Bigl(\frac{\pi}{15}\ell\Bigr)\\
        y_c &= 15\sin\Bigl(\frac{\pi}{15}\ell\Bigr)\\
        z_c &= 1\\
        t_c &= \ell. \label{eq:results-lorentz-circular-motion}
    \end{split}
\end{align}
\begin{figure}[ht!]
    \centering
...
\vtwo{The first visualization is shown in \cref{fig:lorentz-sim-inertial-motion}, where we set $\mathbf{p}=\mathbf{p}_c$ and evaluated $\mathbf{p}'=\mathbf{H}(t)\mathbf{p}$ as well as the elapsed proper time $\Delta \tau = \frac{1}{\gamma}\Delta t$. As can be seen, an observer following the motion of the system -- constantly changing referential frames -- sees a distorted circular motion. The elapsed time for this observer is almost $40\%$ of the ``real'' elapsed time, when at maximum speed.}
\begin{figure}[ht!]
    \centering
    \def\svgwidth{\linewidth}
...
\vtwo{The second visualization is shown in \cref{fig:lorentz-sim-relativistic-motion}, where we set $\mathbf{p}'=\mathbf{p}_c$ and evaluated $\mathbf{p}=\mathbf{H}^{-1}(t)\mathbf{p}'$ as well as the elapsed coordinate time $\Delta t = \gamma\Delta\tau$. As can be seen, an observer in the inertial frame sees the motion of the system as a distorted circular motion, although different. As the elapsed time shows, the inertial observer sees the events happening at a slower rate, almost double the proper elapsed time.}
\begin{figure}[ht!]
    \centering
    \def\svgwidth{\linewidth}
...
\vtwo{A comparison of both the proper time and the coordinate time is shown in \cref{fig:lorentz-sim-time-dilat}. As can be seen, the proper time elapses slower than the coordinate time, which is a direct consequence of time dilation. This effect is more pronounced as the system approaches the speed of light, as expected. }
\begin{figure}[ht!]
    \centering
    \def\svgwidth{\linewidth}
    {\footnotesize\import{figures/}{lorentz_distanceD.pdf_tex}}
    \caption{Value of EC-distance $D$ over time.}
    \label{fig:lorentz-sim-distance-D}
\end{figure}

\vtwo{Finally, we show the value of the EC-distance $D$ in \cref{fig:lorentz-sim-distance-D}. The system successfully converges to the desired curve, achieving a final distance of approximately $0.03$. This result also allows the confirmation of the circulation of the curve, since after convergence we still observe varying dilation effects. The noisy behavior of the distance function is due to the magnitude of the Lorentz transformation matrices components. During the system motion, we observed variations on the $z$ and $y$-axis related components of the Lorentz transformation, although their magnitude was at maximum $\num{1E-4}$. This small magnitude along with the approximations made, propagated as noise in the distance function. Although we set $c=1$ to reduce possible numerical errors, it is still visible that the group $\text{SO}^+(3, 1)$ is a lot more sensitive than $\text{SE}(3)$, and thus require more careful numerical methods.}
