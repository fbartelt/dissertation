...
\vtwo{Finally, the value of the EC-distance $D$ is shown in \cref{fig:lorentz-sim-distance-D}. The system successfully converges to the desired curve, achieving a final distance of approximately $0.03$, which was deemed acceptable but could potentially be reduced by adjusting the gains. This result also confirms the circulation of the curve, as varying dilation effects are still observed after convergence. The noisy behavior in the distance function arises from the magnitude of the Lorentz transformation matrix components. During the system's motion, variations were observed in the $z$- and $y$-axis-related components of the Lorentz transformation, though their magnitudes were at most $\num{1E-4}$. These small variations, combined with the approximations used, propagated as noise in the distance function. Although $c = 1$ was chosen to minimize numerical errors, it remains evident that the group $\text{SO}^+(3, 1)$ is significantly more sensitive to numerical methods than $\text{SE}(3)$, requiring more careful numerical handling.}
