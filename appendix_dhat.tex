...
\vtwo{\begin{align}
    \begin{split}
        ...
        \end{bmatrix}\label{eq:appendix-L-of-trace-OR-Lop-cos-theta-explicit}\\
        &= \frac{1}{2}\mathbf{g}.
    \end{split}
\end{align}}
...
\end{align}}
\vtwo{Now, note that $\Lop[\tr(\cdot)]$ was already computed in \eqref{eq:appendix-L-of-trace-OR-Lop-cos-theta-explicit} -- it is the same as $\Lop[\cos\theta]$ except for the $1/2$ coefficient. Therefore, we can express $\Lop[\sin\theta]$ as:}
\vtwo{\begin{align}
