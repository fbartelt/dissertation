...
\end{align}
As shown in \cref{sec:explicit-construction-SE3}, $\theta$ is expressed as $\text{atan2}(\sin\theta, \cos\theta)$, however both derivatives of $\text{atan2}$ and $\tan^{-1}$ coincide. The $\text{L}$ operator of $\theta$ is then expressed as
\begin{align}
...
\begin{align}
    ...
    \end{bmatrix}.
\end{align}
If we denote $\rot(\mathbf{Z})_{ij}$ as the element in the $i^{th}$ row and $j^{th}$ column of $\rot(\mathbf{Z})$, this expression simplifies to:
\begin{align}
    \Lop\biggl[\frac{1}{2}\Bigl(\tr\bigl(\rot(\mathbf{Z})\bigr)-1\Bigr)\biggr](\mathbf{Z}) = \begin{bmatrix}
...
Switching our attention to the numerator of \eqref{eq:explicit-derivative-SE3-LthetaZ}, we note the following:
\begin{align}
...
\end{align}
using properties of the trace. With this, we can express the $\Lop$ operator of the numerator as:
\begin{align}
...
\end{align}
Finally, using the previously computed $\Lop$ operator for the trace, this simplifies to:
\begin{align}
...
Now, note that the quotient rule for derivatives also applies to the $\Lop$ operator (see \cref{sec:appendix-prop-Lop-product-quotient}). Using the following equivalences:
\begin{align}
