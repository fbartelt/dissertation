...
\section{Chain rules}
A chain rule has already been shown in \cref{lemma:chainrule}, which applies to expressions of the form $\frac{d}{d\sigma}f\bigl(\mathbf{X}(\sigma)\bigr)$. In this section, we present \vtwo{two other chain rules. The first one is the chain rule of the $\Lop$ operator of function composition. The second one is the chain rule related to the ``partial'' $\Lop_\mathbf{V}$ operator for left-invariant functions.}
...
\vtwo{In the context of vector fields, the Lie derivative is a directional derivative of a vector field on a manifold as is defined as follows. Let $G$ be a Lie group and $V\in T_\mathbf{X}G$, $W\in T_\mathbf{Y}G$ vector fields on $G$. Since these vector fields belong to different tangent spaces, it is necessary to transport points by means of the flow. Let $\rho:\mathbb{R}\times G$ be the flow of $V$. We can follow the integral curve given by $\rho$ with initial condition $\mathbf{X}$ and evaluate $W(\rho(t, \mathbf{X}))$, which belongs to the tangent space of $G$ at $\rho(t, \mathbf{X})$. Thus, to compare both vector fields, we need to transport $W\bigl(\rho(t, \mathbf{X})\bigr)$ back to $T_\mathbf{X}G$. The Lie derivative of $W$ with respect to $V$, denoted $\mathcal{L}_V W$, at $\mathbf{X}$ is defined as \citep[p. 316]{Gallier2020}
\begin{align}
    (\mathcal{L}_V W)_\mathbf{X}=\frac{d}{d\sigma}\bigl(d(\rho(-\sigma, \mathbf{X}))_{\rho(\sigma, \mathbf{X})}\bigr)\bigl(W(\rho(\sigma, \mathbf{X}))\bigr)\bigr|_{\sigma=0}.
\end{align}
This operation is commonly expressed in terms of the \emph{pull-back} of $W$ by $\rho$, denoted $\rho(\sigma, \mathbf{X})^*W$, achieving the simpler expression \citep[p. 317]{Gallier2020}
\begin{align}
    (\mathcal{L}_V W)_\mathbf{X} = \frac{d}{d\sigma}\bigl(\rho(\sigma, \mathbf{X})^*W\bigr)\bigr|_{\sigma=0}.
\end{align}}

\vtwo{The Lie derivative of a function $f:G\to \mathbb{R}$ at a point $\mathbf{X}\in G$ is expressed as \citep[p. 121]{Lang2012}:
\begin{align}
    (\mathcal{L}_V f)_\mathbf{X} = \lim_{\sigma \to 0 }\frac{f\bigl(\rho(\sigma, \mathbf{X})\bigr) - f(\mathbf{X})}{\sigma} = \frac{d}{d\sigma}\Bigl(f\bigl(\rho(\sigma, \mathbf{X})\bigr)\Bigr)\Bigr|_{\sigma=0}.
\end{align}}

\vtwo{If $G$ is a matrix Lie group, and $\mathbf{G}(\sigma)$ is a curve in $G$ with tangent vector $V$ at $\mathbf{G}(\sigma)$ and initial condition $\mathbf{X}$, we have
\begin{align}
...
\end{align}
for some isomorphism $\SL:\mathbb{R}^m\to \mathfrak{g}$ and function $\boldsymbol{\xi}:\mathbb{R}\to\mathbb{R}^m$. Thus, a local flow $\rho$ of $V$ can be expressed as $\exp\bigl(\varepsilon\SL[\boldsymbol{\zeta}]\bigr)\mathbf{X}$ for a small $\varepsilon$ and some vector $\boldsymbol{\zeta}\in\mathbb{R}^m$. The Lie derivative of $f$ with respect to $V$ at $\mathbf{X}$ can be expressed as
\begin{align}
    (\mathcal{L}_V f)_{\mathbf{X}} = \lim_{\varepsilon \to 0 }\frac{f\Bigl(\exp\bigl(\varepsilon\SL[\boldsymbol{\zeta}]\bigr)\mathbf{X}\Bigr) - f(\mathbf{X})}{\varepsilon},
\end{align}
which by definition is exactly equal to $\Lop[f](\mathbf{X})\boldsymbol{\zeta}$, which shows that the $\Lop$ operator is a portion of the Lie derivative.}