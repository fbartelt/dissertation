...
In the direct approach the controller parameters $\theta_c$ are directly adapted to control the system $P(\theta)$. This parameterization ensures that when the true controller parameters are used, denoted by $P_c(\theta_c)$, the results are equivalent to the behavior of the true plant. The adaptation law estimates the controller parameters $\widehat{\theta}_c(t)$, which are then used to compute the control signal. A block diagram of this approach is shown in \cref{fig:direct-adaptive-control}.
\begin{figure}[ht]
    \centering
    \begin{tikzpicture}[
        block/.style={draw, rectangle, minimum height=1.2cm, minimum width=2.4cm, align=center},
